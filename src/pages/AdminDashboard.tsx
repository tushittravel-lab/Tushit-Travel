...
const InquiriesTable = ({ showToast }: { showToast: (m: string, t: 'success' | 'error') => void }) => {
  ...
  return (
    <div className="glass-card rounded-3xl overflow-hidden">
      <ConfirmModal 
        ...
        onConfirm={() => confirmDelete && deleteInquiry(confirmDelete.id)}
        onCancel={() => setConfirmDelete(null)}
      />
      <div className="p-10 border-b border-white/5 flex justify-between items-center">
        <h3 className="text-xl font-black tracking-tight">Contact Inquiries</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-white/5">
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Name</th>
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Contact</th>
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Subject</th>
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Message</th>
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Received</th>
              <th className="p-8 text-[10px] font-black uppercase tracking-widest opacity-40">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {inquiries.map((inquiry) => (
              <tr key={inquiry.id} className="hover:bg-white/[0.02] transition-colors group">
                ...
                <td className="p-8">
                  ...
                  <p className="opacity-20 text-xs mt-1">{inquiry.phone}</p>
                </td>
                <td className="p-8">
                  <p className="font-bold text-sm">{inquiry.subject}</p>
                </td>
                <td className="p-8 max-w-xs">
                  <p className="opacity-60 text-xs line-clamp-3">{inquiry.message}</p>
                </td>
                <td className="p-8">
                ...
                <td className="p-8">
                  <button 
                    onClick={() => setConfirmDelete({ id: inquiry.id })}
                    className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center opacity-40 hover:text-red-500 transition-colors"
                  >
                    <XCircle size={16} />
                  </button>
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto h-screen no-scrollbar relative z-10">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'inquiries' && <InquiriesTable showToast={showToast} />}
